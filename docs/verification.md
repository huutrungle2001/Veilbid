...
| Finalizer | Dry-run, race, budget, health, sanitized logs | PARTIAL — 12 tests cover planning, canonical rereads, sequential shared budget, stale races, sanitized failures, configuration, and health; live Sepolia health/dry-run pass while close/finalize writes remain pending |
...
| Frontend unit | Wallet, forms, roles, URL state, pending/recovery | PARTIAL — 30 tests cover wallet discovery/reconnect/network/plaintext clearing, Buyer/Vendor validation, URL-selected roles and dossiers, loading/failure states, sanitized recovery checkpoints, bounded proof retry, Activity recovery, and ACL-before-decrypt Auditor flow; fresh-wallet live browser writes/reveal remain pending |
