...
| Frontend unit | Wallet, forms, roles, URL state, pending/recovery | PARTIAL — 33 tests cover wallet discovery/reconnect/network/plaintext clearing, Buyer/Vendor validation, URL-selected roles and dossiers, loading/failure states, recovery, ACL-before-decrypt Auditor flow, and preparation-only Safe term binding; fresh-wallet live browser writes/reveal remain pending |
