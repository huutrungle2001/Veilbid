...
| Frontend unit | Wallet, forms, roles, URL state, pending/recovery | PARTIAL — 26 tests cover wallet discovery/reconnect/network clearing, Buyer/Vendor validation, URL-selected public dossiers, loading/failure states, sanitized recovery checkpoints, bounded proof retry, and Activity recovery decisions; fresh-wallet live browser writes remain pending |
