...
| Finalizer | Dry-run, race, budget, health, sanitized logs | PASS — 12 tests cover planning, canonical rereads, sequential shared budget, stale races, sanitized failures, configuration, and health; verified release health/dry-run and relay-originated live close/finalize writes pass on Sepolia tender #5 |
...
| Public index | Rebuild, checkpoint, bounded logs, reorg/failure path | PARTIAL — browser, relay, and MCP share provider-compatible 1,000-block finalized Sepolia RPC pagination; deterministic rebuild/dedupe/guards pass and live relay/MCP reads recover correctly; a provider reorg drill remains pending |
| Frontend unit | Wallet, forms, roles, URL state, pending/recovery | PARTIAL — 57 tests cover wallet/session clearing, Buyer/Vendor validation, standalone routes, shared public pagination, public dossiers/receipts, canonical release labeling, recovery, per-role disclosure filters, Auditor ACL-before-decrypt, Safe term binding, and full target/calldata/JSON Safe handoff; fresh-wallet live browser writes/reveal remain pending |
...
| 2026-07-26 | Settlement relay write lifecycle | `3d2093a` | `evidence/sepolia/relay-write-e2e.json` | PASS — canonical Safe funded tender #5, a vendor submitted an encrypted bid, and the relay account closed and proof-finalized the proof-derived award; failed setup tenders #3/#4 were cancelled and their escrow returned to the Safe |
...
| 2026-07-26 | Tender Room Safe Treasury | `e96586a` | 33 Tender Room tests, Vite build, and live 1440×1000 local smoke | PARTIAL — full terms/consumer/nonce feed the preparation-only module; the canonical module is enabled and the UI now exposes complete target/calldata handoff for normal Safe authorization, while a fresh owner-wallet preparation remains pending |
