...
| Public index | Rebuild, checkpoint, bounded logs, reorg/failure path | PARTIAL — deterministic rebuild/dedupe/guards and provider-compatible 1,000-block finalized Sepolia RPC pagination pass; reorg rollback pending |
| Frontend unit | Wallet, forms, roles, URL state, pending/recovery | PARTIAL — 46 tests cover wallet/session clearing, Buyer/Vendor validation, standalone routes, provider-bounded public pagination, public dossiers/receipts, canonical release labeling, recovery, per-role disclosure filters, Auditor ACL-before-decrypt, and preparation-only Safe term binding; fresh-wallet live browser writes/reveal remain pending |
