...
| Navigation scroll reset | Verified | Agent | 2026-07-29 | Primary page links, Tender Room workspaces, and Buyer/Private Bids subviews now reset the viewport to the page top on selection, including same-page navigation. Dossier, award-history, and Docs anchor actions retain their targeted scrolling. Full repository tests (106 web tests; 176 total), lint/build, production desktop/mobile smoke, and keyboard/contrast smoke passed on Vercel deployment `HcBrVP8PDVBrtphHnE7opN6g6fNB` from source commit `5b8abc4`; local `VEILBID.mp4` remains untracked and is explicitly excluded from Vercel uploads. |
