...
| Compact Safe tender balance | Approved for next UI polish | User | 2026-07-28 | Replace the large balance-validation card inside Safe tender terms with one compact `vcUSDC` value and eye control beside `TENDER TERMS / Public procurement rules`. Preserve reveal-required and ceiling-within-balance validation while removing the oversized frame and explanatory copy. |
| Contextual workflow guidance | Implemented, pending release | Agent | 2026-07-28 | Added per-card hover/focus `?` help across Public, EOA/Vendor/Disclosure/Auditor, Safe treasury actions, and Activity recovery cards; Public now keeps the dossier list and selected detail independently visible on desktop. Confidential bid reveals format six-decimal vUSDC units instead of raw micro-units. |
