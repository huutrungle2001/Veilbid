...
monospace carries handles, proofs, blocks, ACLs, and timestamps. Surfaces remain
...
- **Public winner, private price.** Every screen distinguishes public metadata,
  encrypted values, authorized plaintext, and publicly verified results.
- **Roles before controls.** Buyer, Vendor, Public Finalizer, Auditor, and Safe
  Signer contexts must be explicit before an action appears.
- **Proof is a journey.** Wallet preparation, Sepolia confirmation, Nox
  indexing, public proof, settlement, and refreshed reveal are separate stages.
...
- Weight: 400 or 700.
- Role: eyebrows, privacy labels, timestamps, chain IDs, addresses, handles,
  proofs, transaction hashes, ACL entries, and receipt metadata.
...
- Pre-submit checklist: correct role, Sepolia, balance/operator, deadline, target.
...
- Mono eyebrow: `PUBLIC PROOF / PRIVATE PRICE`.
...
4. Nox indexing.
5. Proof/decryption.
6. State and authorized reveal refreshed.

Mined transactions are not marked failed because a later Nox stage is pending.
...
`CONFIDENTIAL PROCUREMENT / ETHEREUM SEPOLIA`
...
The illustration depicts sealed bid envelopes moving through a black Nox
aperture toward a green award stamp. Use flat cel-shaded forms, thick ink
outlines, no photographic texture, and no token/coin imagery.

### Marquee

```text
ENCRYPTED BIDS — PROOF-DERIVED WINNERS — SAFE-OWNED BUDGETS —
SELECTIVE AUDIT — PERMISSIONLESS FINALIZATION
```
...
- Product statement.
- Tenders, Docs, Evidence, GitHub, and Sepolia links.
- Social links.
...
- Handle and address truncation always exposes a labelled copy/full-value
  control.
...
- Do not merge Safe ownership with connected-wallet identity.
- Do not describe encrypted handles as anonymous transactions.
...
> distinction between connected wallet, Safe owner, buyer, vendor, finalizer,
> and auditor.
...
> Tender cards show public ceiling, deadline, buyer/Safe, bid count, lifecycle,
...
> white numeric field, target/ACL explanation, public ceiling and deadline, and
> a green `ENCRYPT & SUBMIT BID` pill. Do not place plaintext bid values in URL,
> activity, analytics, or completed-state copy.

### Winner panel prompt

> Create a full-bleed black award panel with mono eyebrow `PUBLIC PROOF /
> PRIVATE PRICE`, a thin serif `AWARDED` headline, public winner identity, proof
> and receipt evidence, and an encrypted winning-price badge unless the viewer
> is authorized. A single green halo may sit behind the award seal.

### Safe header prompt

> Create a flat Safe custody panel with address, signer, owners, threshold,
> restricted-module status, confidential budget, and operator state. Include
> the exact message `THE SAFE OWNS THE FUNDS. THIS WALLET IS A SIGNER.` Use no
> dashboard sidebar and no decorative elevation.

### Proof inspector prompt

> Create a white flat proof inspector using Space Mono for chain ID, block,
> contract, transaction, winner-handle fingerprint, verification result, and
> receipt. Never expose full proof bytes, signatures, keys, or confidential
> plaintext.
